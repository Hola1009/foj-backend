package com.fancier.foj.gateway.filter;
...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (matches(url, ignoreWhite.getWhites())) {
            ...
            return chain.filter(exchange); // 乳沟 url 在白名单中则不许要直行下列过滤器的代码
        }
        ...
        String userId = JwtUtils.getUserId(claims);  //获取jwt中的key
        boolean isLogin = redisService.hasKey(getTokenKey(userId));         //7c114ab4-e4d7-4392-8630-3e248a9cb335         //42752c9a-009a-47bb-8a9c-1d34f4287944
        if (!isLogin) {
